...
export async function getProjectContributions(
  ...
  const [
    ...
    supabase
      .from("annotations")
      .select(
        "id, project_work_id, created_by, created_at, comment, project_works!inner(works(title))",
      )
      .eq("project_id", projectId)
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
  ...
  if (extractionValuesData.data) {
    const rawExtractionValues = extractionValuesData.data as unknown as Array<{
      extractions?: { extractor_id?: string } | Array<{ extractor_id?: string }>;
    }>;
    for (const row of rawExtractionValues) {
  ...
  interface ScreeningDecisionDbRow {
    id: string;
  ...
  const screeningDecisions: RawScreeningDecision[] = (
    (decisionsData.data ?? []) as unknown as ScreeningDecisionDbRow[]
  ).map((d) => ({
    id: d.id,
  ...
  const extractions: RawExtraction[] = (
    (extractionsData.data ?? []) as unknown as ExtractionDbRow[]
  ).map((e) => ({
    id: e.id,
  ...
  const projectWorks: RawProjectWork[] = (
    (projectWorksData.data ?? []) as unknown as ProjectWorkDbRow[]
  ).map((pw) => ({
    id: pw.id,
  ...
  const annotations: RawAnnotation[] = (
    (annotationsData.data ?? []) as unknown as AnnotationDbRow[]
  ).map((a) => ({
    id: a.id,
  ...
  const reconciliations: RawReconciliation[] = (
    (reconciliationsData.data ?? []) as unknown as ReconciliationDbRow[]
  ).map((r) => ({
  ...
  let rawAuthRows = (authEventsData?.data ?? []) as unknown as AuthEventDbRow[];
    ...
    try {
      ...
      if (admin) {
        ...
        if (adminAuthRes.data && adminAuthRes.data.length > 0) {
          rawAuthRows = adminAuthRes.data as unknown as AuthEventDbRow[];
        }
      }
    } catch {
      // Non-blocking
  ...
  try {
    ...
    if (admin) {
      ...
      if (devicesRes.data) {
        devices = (devicesRes.data as unknown as RawDevice[]).map((d) => ({
          id: d.id,
          user_id: d.user_id,
          label: d.label,
          created_at: d.created_at,
