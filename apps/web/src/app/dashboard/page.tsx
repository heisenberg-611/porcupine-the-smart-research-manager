...
export default async function DashboardPage() {
  ...
  const [projectData, progressData, queueData, memberData] = await Promise.all([
    ...
    must(supabase.from("project_members").select("project_id"), "members"),
  ]);
  ...
  return (
    <main
      id="main"
      className="animate-in fade-in slide-in-from-bottom-4 mx-auto flex max-w-4xl flex-col gap-10 px-6 py-12 duration-700"
    >
      <PageHeader
        title="Dashboard"
                ...
                return (
                  <li key={project.id} className="group">
                    <Link href={`/projects/${project.id}`} className="block h-full">
                      <div className="border-rule bg-surface/40 hover:bg-accent/5 hover:border-accent/40 flex h-full flex-col justify-between gap-4 rounded-2xl border p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex min-w-0 items-center gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-muted text-fine mt-0.5 flex flex-wrap items-center gap-1.5 truncate">
                                  <span>
                                    {project.kind.replace(/_/g, " ").toLowerCase()}
                                  </span>
                                  <span className="opacity-50">•</span>
                                  <span className="capitalize">
                                    {project.ownership_model
                                      .replace(/_/g, " ")
                                      .toLowerCase()}
                                  </span>
                                  <span className="opacity-50">•</span>
              ...
              {totalUndecided > 0 ? (
                <>
                  <strong className="text-ink">
                    {totalUndecided} {totalUndecided === 1 ? "paper is" : "papers are"}
                  </strong>{" "}
                  undecided across your projects.
                </>
              ) : (
  ...
  return (
    <li>
      <Link
        ...
        className="group border-rule bg-surface/30 hover:border-accent/40 hover:bg-accent/5 focus-visible:ring-accent relative block overflow-hidden rounded-2xl border p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg focus-visible:ring-2 focus-visible:outline-none"
      >
        <span className="text-muted text-ui group-hover:text-ink relative z-10 block font-medium transition-colors">
          {label}
        </span>
        <span
          className={`text-display relative z-10 mt-2 block font-serif tabular-nums ${
        ...
        <div className="from-accent/5 absolute -inset-x-4 -bottom-4 h-1/2 bg-gradient-to-t to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
      </Link>
    </li>
  );
}
