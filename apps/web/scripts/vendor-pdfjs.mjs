 ...
 *
 * Run from `dev`, `build` and `postinstall`, which is cheap because repeat
 * runs exit on the version stamp below in about forty milliseconds.
 *
 * postinstall is the honest trigger — these files are copies of a dependency,
 * so the moment they can go stale is the moment that dependency changes — but
 * `dev` and `build` call it too, so a `public/` that was cleaned by hand or
 * missed by a cached CI job cannot produce a 404 for the worker with nothing
 * pointing back here.
 *
 * `pnpm --filter @Porcupine/web vendor:pdfjs` runs it directly.
 */
import {
  cpSync,
  copyFileSync,
 ...
 *
 * 2.4 MB of fonts and cmaps copied on every build is not free, and this runs
 * from postinstall as well. The stamp is the version, so an upgrade
 * invalidates it and nothing else does.
 */
const stamp = join(target, ".version");
const styles = join(app, "src", "styles");
const layerCss = join(styles, "pdf-text-layer.css");

/*
 * Every output, not just the stamp.
 *
 * This script writes into two places — `public/pdfjs` and `src/styles` — and
 * both are gitignored. Checking only the stamp means deleting the stylesheet
 * leaves a version file claiming the work is done, and the next build fails on
 * a missing import with nothing pointing back here.
 */
const upToDate =
  existsSync(stamp) &&
  readFileSync(stamp, "utf8") === version &&
  existsSync(join(target, "pdf.worker.min.mjs")) &&
  existsSync(layerCss);

if (upToDate) {
  console.warn(`pdf.js ${version} already vendored`);
  process.exit(0);
}
